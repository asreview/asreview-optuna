...
def objective(trial):
    ...
    for i, row in studies.iterrows():
        ...
        losses.append(loss(padded_results))

    return np.mean(losses)


class StopWhenOptimumReached:
    def __init__(self, min_trials: int, threshold: int, history: int):
    ...
    def __call__(
        ...
        if trial.number > self.min_trials:
            # Take history num of trials (hence history + 1)
            prev_trials = study.trials[1 : self.history + 1]

            # Calculate mean loss value of the prev_trials
            mean_loss = np.mean([prev_trial.value for prev_trial in prev_trials])

            # If the difference is smaller than threshold, we stop the entire study
            if abs(mean_loss - trial.value) < self.threshold:
                study.stop()
...
if __name__ == "__main__":
    ...
    study_stop_cb = StopWhenOptimumReached(
        min_trials=MIN_TRIALS, threshold=STOPPING_THRESHOLD, history=HISTORY
    )
